...
const SearchCharForm = ({ getCharForName }) => {
  ...
  const searchChar = async (e) => {
    ...
    if (charName.trim().length === 0)
      return setNotFound("This field is required");
    try {
      const res = await fetch(
        `${BASE_URL}characters?name=${charName}&${API_KEY}`
      );
  ...
  const content = (
    <div className="char__form-bottom__block">
  ...
  return (
    <form onSubmit={searchChar} className="char__form">
      ...
      <div className="char__form-mobile">
        <input
          value={charName}
